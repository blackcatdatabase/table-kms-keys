 ...
 * - whitelist LIKE:   [ 'external_key_ref', 'purpose', 'algorithm', 'status' ]
 * Podporované tvary:
...
final class Criteria {
    ...
    public function toSql(bool $viewMode = false): array {
        ...
        $allowed = array_fill_keys([ 'id', 'provider_id', 'external_key_ref', 'purpose', 'algorithm', 'status', 'created_at' ], true);
        ...
        $order = null;
        if ($this->sort) {
            $safeCols = array_fill_keys([ 'id', 'provider_id', 'external_key_ref', 'purpose', 'algorithm', 'status', 'created_at' ], true);
            $parts = [];
