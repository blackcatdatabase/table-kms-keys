...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'provider_id', 'external_key_ref', 'purpose', 'algorithm', 'status', 'created_at' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'external_key_ref', 'algorithm' ];
    }

    /** Columns allowed in ORDER BY (falls back to filterable() when empty). */
    protected function sortable(): array
    {
        $x = [ 'id', 'provider_id', 'external_key_ref', 'purpose', 'algorithm', 'status', 'created_at' ];
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\KmsKeys\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\KmsKeys\Definitions::softDeleteColumn();
