...
namespace BlackCat\Database\Packages\KmsKeys\Joins;
...
final class KmsKeysJoins {
    ...
    /**
     * FK: kms_keys -> kms_providers
     * LEFT JOIN vw_kms_providers AS $as ON $as.id = $alias.provider_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinKmsProviders(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_kms_providers AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.provider_id' . ' ', []];
    }
