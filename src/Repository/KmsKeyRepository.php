...
use BlackCat\Database\Packages\KmsKeys\Repository\KmsKeyRepositoryInterface;
...
class KmsKeyRepository implements KmsKeyRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'provider_id', 'external_key_ref' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\KmsKeys\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'purpose', 'algorithm', 'status' ]);
          return count($rows);
        }
    ...
    public function getByProviderIdAndExternalKeyRef(int $providerId, string $externalKeyRef, bool $asDto = false): array|\BlackCat\Database\Packages\KmsKeys\Dto\KmsKeyDto|null {
        return $this->getByUnique([ 'provider_id' => $providerId, 'external_key_ref' => $externalKeyRef ], $asDto);
    }
    ...
    public function getIdByProviderIdAndExternalKeyRef(int $providerId, string $externalKeyRef) {
        $row = $this->getByProviderIdAndExternalKeyRef($providerId, $externalKeyRef, false);
