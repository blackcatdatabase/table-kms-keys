...
namespace BlackCat\Database\Packages\KmsKeys\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\KmsKeys\Definitions;
use BlackCat\Database\Packages\KmsKeys\Criteria;
use BlackCat\Database\Packages\KmsKeys\Dto\KmsKeyDto as Dto;
use BlackCat\Database\Packages\KmsKeys\Mapper\KmsKeyDtoMapper as RowMapper;
...
final class KmsKeyRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\KmsKeys\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'purpose', 'algorithm', 'status' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'provider_id', 'external_key_ref' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\KmsKeys\Definitions::table());
            return $bulk->upsertMany($rows, [ 'provider_id', 'external_key_ref' ], [ 'purpose', 'algorithm', 'status' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KmsKeys\Dto\KmsKeyDto|null */
    public function getByProviderIdAndExternalKeyRef(int $providerId, string $externalKeyRef, bool $asDto = false): array|\BlackCat\Database\Packages\KmsKeys\Dto\KmsKeyDto|null {
        $row = $this->getByUnique([ 'provider_id' => $providerId, 'external_key_ref' => $externalKeyRef ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\KmsKeys\Mapper\KmsKeyDtoMapper::fromRow($row);
    }
    public function existsByProviderIdAndExternalKeyRef(int $providerId, string $externalKeyRef): bool {
        $where = 't.' . Ident::q($this->db, 'provider_id') . ' = :uniq_provider_id' . ' AND ' . 't.' . Ident::q($this->db, 'external_key_ref') . ' = :uniq_external_key_ref';
        return $this->exists($where, [ 'uniq_provider_id' => $providerId, 'uniq_external_key_ref' => $externalKeyRef ]);
    }
    /** @return int|string|null */
    public function getIdByProviderIdAndExternalKeyRef(int $providerId, string $externalKeyRef) {
        $row = $this->getByProviderIdAndExternalKeyRef($providerId, $externalKeyRef);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KmsKeys\Dto\KmsKeyDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\KmsKeys\Dto\KmsKeyDto|null {
